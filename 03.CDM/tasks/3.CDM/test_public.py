...
import pytest
from cdm import get_table_path
import random
import os
...
def test_ods(test_case: Case) -> None:
    ...
    rows = yt.read_table(path_test + "[:50]")
    for i in rows:
        h = hash(str(i))
        if h in test:
            test.remove(h)
    ...
    for i in range(1, 10):
        keys.append(str(random.randint(2, 1000)))
